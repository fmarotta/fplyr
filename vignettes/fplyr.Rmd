...
Another useful option is `parallel`, with which it is possible to specify the number of threads that `fplyr` can use. Like `nblocks`, also the `parallel` option is supported by all the functions. It is not necessary to initialise any cluster. In the following example we will select, for each block, a random sample of 10 observations.
